...
daCount = 0
verb, verb2, noun, adj, noun2, finisher = getCompliments()
reactTo,channelIDtoMsg,daGuildID = getScope()
msgAsReply = getMisc()

def createCompliment():
    daMessage = ""
    daMessage += verb[random.randint(0,len(verb))-1]+" "+verb2[random.randint(0,len(verb2))-1]+" "+noun[random.randint(0,len(noun))-1]+" "+adj[random.randint(0,len(adj))-1]+" "+noun2[random.randint(0,len(noun2))-1]+" "+finisher[random.randint(0,len(finisher))-1]
    return daMessage
